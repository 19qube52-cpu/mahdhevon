...
import { evaluateFormula, resultBreakdown, validateInputs, type DynamicCalculatorDefinition } from "@/lib/dynamic-calculator"
...
export default function BlankCalculatorPage({ slug }: { slug: string }) {
  ...
  const errors = useMemo(() => definition ? validateInputs(definition, values) : {}, [definition, values])
  ...
  const breakdown = result === null || Object.keys(errors).length ? [] : resultBreakdown(result, config.unit)
  return <main className="mx-auto max-w-6xl px-4 py-8" dir="rtl">
    ...
    <section className="mt-6 overflow-hidden rounded-3xl border bg-card shadow-sm">
      <div className="grid lg:grid-cols-2">
        <header className="flex flex-col justify-center bg-gradient-to-bl from-primary/15 via-background to-background p-8 sm:p-12">
          ...
          <div className="flex items-center gap-2"><h1 className="text-3xl font-black sm:text-5xl">{title}</h1>{definition.status === "draft" ? <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-800">טיוטה</span> : null}</div>
          <p className="mt-5 text-lg leading-8 text-muted-foreground">{definition.description}</p>
        </header>
        {imageUrl ? <img src={imageUrl} alt={title} className="h-full min-h-72 w-full object-cover" /> : <div className="flex min-h-72 items-center justify-center bg-gradient-to-br from-slate-950 via-indigo-950 to-primary"><Calculator className="h-24 w-24 text-amber-300/80" /></div>}
      </div>
    </section>
    <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_.85fr]">
      <section className="rounded-3xl border bg-card p-6 shadow-sm sm:p-8">
        ...
        <div className="space-y-5">{definition.inputs.map(input => <label key={input.id} className="block"><span className="mb-2 block font-bold">{input.label}</span><div className="relative"><input type={input.type} min={input.min} max={input.max} step={input.step ?? 1} value={values[input.id] ?? ""} onChange={event => setValues(current => ({ ...current, [input.id]: Number(event.target.value) }))} className={`w-full rounded-xl border bg-background px-4 py-3 text-lg outline-none focus:ring-2 ${errors[input.id] ? "border-destructive focus:ring-destructive" : "focus:ring-primary"}`} />{input.unit && input.type === "number" ? <span className="absolute left-4 top-3.5 text-muted-foreground">{input.unit}</span> : null}</div>{errors[input.id] ? <span className="mt-1 block text-sm text-destructive">{errors[input.id]}</span> : input.helpText ? <span className="mt-1 block text-sm text-muted-foreground">{input.helpText}</span> : null}</label>)}</div>
      </section>
      <aside className="rounded-3xl bg-primary p-7 text-primary-foreground shadow-xl sm:p-9"><p className="text-sm font-bold opacity-80">{config.label ?? "התוצאה"}</p><div className="mt-4 break-words text-4xl font-black sm:text-5xl">{result === null || Object.keys(errors).length ? "—" : `${config.prefix ?? ""}${result.toLocaleString("he-IL", { maximumFractionDigits: Math.min(2, Math.max(0, config.decimals ?? 0)) })}${config.unit ? ` ${config.unit}` : ""}${config.suffix ?? ""}`}</div><div className="mt-6 grid grid-cols-2 gap-2">{breakdown.slice(1).map(item => <div key={item.label} className="rounded-xl bg-white/10 p-3"><div className="text-xs opacity-70">{item.label}</div><div className="mt-1 font-bold">{item.value.toLocaleString("he-IL", { maximumFractionDigits: 1 })}</div></div>)}</div><p className="mt-6 text-sm leading-6 opacity-80">התוצאה מתעדכנת אוטומטית לפי הנתונים שהזנת.</p></aside>
    </div>
