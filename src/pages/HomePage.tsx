...
export default function HomePage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="bg-gradient-to-br from-primary/5 via-white to-blue-50 border-b border-border py-10 sm:py-14" aria-label="כותרת ראשית">
            ...
            <div>
              ...
              <div className="relative max-w-md">
                ...
                <input
                  ...
                  className="w-full pr-12 pl-4 py-3.5 text-base border-2 border-border rounded-xl bg-white text-foreground focus:outline-none focus:border-primary shadow-sm transition-shadow"
                  aria-label="חיפוש מחשבון"
                />
                {query.length >= 2 && (
                  <div className="absolute top-full mt-1 w-full bg-white border border-border rounded-xl shadow-xl z-20">
                ...
                {["bruto-neto", "mortgage-payment", "bmi", "pension-estimate"].map((slug) => {
                  ...
                  return (
                    <Link
                      key={slug}
                      to={`/calculators/${slug}`}
                      className="text-sm px-3 py-1.5 bg-white border border-border rounded-full font-medium text-foreground hover:border-primary hover:text-primary transition-colors"
                    >
                      {calc.shortTitle}
                    </Link>
                  )
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {popular.slice(0, 10).map((calc) => (
              <Link
                key={calc.id}
                to={`/calculators/${calc.slug}`}
                className="flex flex-col items-center gap-2 p-4 bg-white border border-border rounded-xl hover:border-primary/50 hover:shadow-sm transition-all text-center group"
              >
